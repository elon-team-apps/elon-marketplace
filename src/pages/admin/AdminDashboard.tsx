...
import {
  ...
  BarChart2, ChevronDown, Calendar, Mail
} from "lucide-react";
import {
  ...
  return (
    <div className="glass-card p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-heading font-semibold text-sm text-foreground">Email Broadcast (Resend)</h2>
      </div>
      <p className="text-xs text-muted-foreground">Send an announcement email to all registered users.</p>
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="text-xs font-semibold text-foreground mb-1 block">Email Body (HTML/Text)</label>
          <textarea
            value={htmlBody}
            onChange={(e) => setHtmlBody(e.target.value)}
            placeholder="<p>Hello! Check out our new update...</p>"
            className="w-full h-32 rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-accent"
          />
        </div>
      </div>

      <div className="flex justify-end pt-2">
        <Button onClick={handleSend} disabled={sending} className="h-9 gap-2">
