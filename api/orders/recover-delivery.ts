...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey) {
    res.status(500).json({ error: "Server misconfigured for delivery recovery." });
    return;
  }
  ...
  const { data: tx, error: txError } = await adminClient
    .from("transactions")
    .select("id, user_id, status, delivered_data")
    .eq("id", transactionId)
    .maybeSingle();
  if (txError || !tx) {
  ...
  if (String(tx.user_id ?? "") !== userData.user.id) {
    res.status(403).json({ error: "You can only recover your own order delivery." });
    return;
  }
  ...
  if (!["completed", "success", "finalized"].includes(status)) {
    res.status(409).json({ error: "Order is still processing. Try again shortly." });
    return;
  }
  ...
  if (!result.ok) {
    res.status(result.status).json({ error: result.error ?? "Delivery recovery failed." });
    return;
  }
