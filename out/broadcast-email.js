...
// api/admin/broadcast-email.ts
var broadcast_email_exports = {};
__export(broadcast_email_exports, {
  default: () => handler
});
module.exports = __toCommonJS(broadcast_email_exports);
...
async function handler(req, res) {
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabaseAdmin.from("profiles").select("role, is_admin").eq("id", user.id).single();
    ...
    const { data: users, error: usersError } = await supabaseAdmin.from("profiles").select("email").not("email", "is", null);
    if (usersError) {
