-- Migration: Fix raw content delivery
-- Ensures that bulk_upload_logs respects the exact format provided and sets raw content explicitly to ensure it acts as the primary data point.
...
BEGIN
  ...
  FOR v_i IN 0..(v_len - 1) LOOP
    ...
    IF v_has_content_col THEN
      ...
      VALUES (p_product_id, raw_content, raw_content, NULL, NULL, NULL, 'available', FALSE);
    ELSE
      INSERT INTO public.log_items (product_id, credentials, email, password, recovery, status, is_delivered)
      VALUES (p_product_id, raw_content, NULL, NULL, NULL, 'available', FALSE);
    END IF;
